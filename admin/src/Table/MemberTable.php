...
class MemberTable extends Table implements VersionableTableInterface, TaggableTableInterface
{
    ...
    {
        $this->typeAlias = 'com_mail_image.member';
    ...
    public function store($updateNulls = true)
    {
        ...
        $table = Table::getInstance('MemberTable', __NAMESPACE__ . '\\', ['dbo' => $this->getDbo()]);

        if ($table->load(['alias' => $this->alias]) && ($table->id != $this->id || $this->id == 0)) {
            // Is the existing Member trashed?
    ...
    public function check()
    {
        ...
        if (trim($this->name) == '') {
            $this->setError(Text::_('COM_MAIL_IMAGE_WARNING_PROVIDE_VALID_MEMBER'));
